      ...
      <div className="flex flex-col gap-10 md:gap-20 p-5 md:px-10 py-20">
        ...
        <section className="flex flex-col md:flex-row gap-5 items-center section">
          <div className="w-full md:w-1/2 flex flex-col gap-5 text-[#29166F]">
            ...
            <p
              className={`${
                service === "Petroleum Products" ? "flex" : "hidden"
              }`}
            >
              ECPROD Nig Ltd is involved in the sourcing, supply, and
              distribution of high-quality petroleum products, catering to the
              energy needs of various industries and sectors. We are committed
              to reliable and efficient product delivery.
            </p>
          </div>
          ...
          <div className="w-fit">
            <div
              className={`mb-3 flex-col ${
                service === "Skill Training" ? "flex" : "hidden"
              }`}
            >
              <h2 className={` text-3xl mb-3 text-[#29166F]`}>
                Our Approach to Skills Training
              </h2>
              <p>
                We adopt a holistic and practical approach to skills training,
                focusing on:
              </p>
            </div>
            <ul
              className={`flex flex-col gap-5 max-w-[600px] pl-5 transition-all duration-500 ease-in-out overflow-hidden list-disc ${
                collapse ? "min-h-screen" : "max-h-screen"
              }`}
            >
              {capabilities.map((item, index) => (
                <li key={index}>
                  <p className="font-semibold inline">{item.subHeading}</p>
                  {item.item}
                  <div
                  ...
                  >
                    <ul
                      className={`flex-col list-disc ml-5 ${
                        item.subHeading === "Flexible Delivery Options: "
                          ? "flex"
                          : "hidden"
                      }`}
                    >
