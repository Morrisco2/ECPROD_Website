...
const Form = () => {
    ...
    return (
        <div className='flex justify-center lg:justify-end flex-grow my-3 mx-2 '>
            <form onSubmit={handleSubmit} className='bg-gray-200 flex flex-col gap-4 rounded-2xl w-96  py-6 px-9 '>
                ...
                {responesSMS && (
                    ...
                    >
                        ...
                        {status !== "sending" && ( 
                            <button type="button" onClick={handleDismiss} className="ml-3 text-xs font-bold" >✕</button>
                        )}
                    </div>
                )}

                <div>
                    <input required onChange={handleChange} name='name' value={input.name} type="text"
                ...
                <div>
                    <input required type="email" onChange={handleChange} name='email' value={input.email}
                        className='w-full h-9 text-sm bg-transparent outline-none border-b-2 border-gray-500 transition duration-100 ease-in hover:border-[#29166F]'
                        placeholder='Email Address' />
                </div>
                <div>
                    <input required type="text" onChange={handleChange} name='subject' value={input.subject}
                        className='w-full h-9 text-sm bg-transparent outline-none border-b-2 border-gray-500 transition duration-100 ease-in hover:border-[#29166F]'
                        placeholder='Subject' />
                </div>
                <div>
                    <textarea required onChange={handleChange} name="message" value={input.message} rows={3}
